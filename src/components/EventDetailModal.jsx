...
const EventDetailModal = ({ isOpen, onClose, event, userRole }) => {
	...
	const fetchFullEventData = async () => {
		...
		try {
			...
			const response = await axios.get(
				`http://localhost:3000/api/events/${event.id_event}`,
				{
					headers: {
	...
	// Функция для вычисления статуса по датам события
	const getStatusByDate = (startDateStr, endDateStr) => {
		if (!startDateStr) return 'Не указан'
		const now = new Date()
		const startDate = new Date(startDateStr)
		const endDate = endDateStr ? new Date(endDateStr) : startDate

		if (endDate < now) return 'архив'
		if (startDate > now) return 'запланировано'
	...
	const dynamicStatus = getStatusByDate(
		displayData.start_datetime,
		displayData.end_datetime
	)
					...
					<>
						<div className='modal-header'>
							<h2 className='modal-title'>
								{displayData.event_name || 'Название не указано'}
							</h2>
							...
							<div className='event-detail-grid'>
								...
								<div className='form-row'>
									...
									<div className='form-group form-col'>
										...
										<p className='form-control-static'>{dynamicStatus}</p>
									</div>
								</div>
											...
											{displayData.participants.map(participant => (
												...
												>
													...
													{participant.participation_status && (
														<span
															className={`participation-status ${participant.participation_status}`}
														>
															{participant.participation_status}
														</span>
													)}
